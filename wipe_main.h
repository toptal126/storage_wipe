...
//    char *s_type = removable ? "Removable" : "Fixed";
    ...
    char charRes[100];
    char *s_type = new char[20];
    if (removable)
        strcpy(s_type, "Removable");
    else strcpy(s_type,"Fixed");
    sprintf(charRes, format_str, szTmp, size, s_type, partType);
    result = result + QString("%1")
            .arg(charRes);
}
...
//			printf("%2d: %s\n", i + 1, device[i]);
    ...
    if (opt.random == RANDOM_WINDOWS) {
        ...
        if (!CryptAcquireContextA(&hProv, KeyContainerName, NULL, PROV_RSA_FULL, 0)) {
